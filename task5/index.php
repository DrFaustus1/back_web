...
header('Content-Type: text/html; charset=UTF-8');
  ...
  session_start() && !empty($_SESSION['login'])) {
    ...
    try{
      ...
      $sth = $db->prepare("SELECT * FROM Form5BD WHERE id = ?");
...
if (empty($_POST['name'])) {
  ...
  if (!empty($_COOKIE[session_name()]) &&
    ...
    try {
      ...
      $stmt = $db->prepare("UPDATE Form5BD SET name = ?, email = ?, year = ?, gender = ?, bio = ? WHERE id =?");
      $stmt -> execute(array(
          $_POST['name'],
          $_POST['email'],
          $_POST['year'],
    ...
    try {
      $stmt = $db->prepare("INSERT INTO Form5BD SET name = ?, email = ?, year = ?, gender = ?, bio = ?");//, login = ?, password = ?
      $stmt -> execute(array(
          $_POST['name'],
          $_POST['email'],
          $_POST['year'],
