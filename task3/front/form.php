...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if($action == 'delete'){
        try {
            ...
            $rmUsers = $db->prepare("DELETE FROM Form6BD WHERE id=:id");
...
try {
  ...
  WHERE NOT EXISTS (
    ...
    $allusers =  $db->prepare("SELECT * FROM Form6BD");
...
if (empty($_SERVER['PHP_AUTH_USER']) ||
    ...
    if(empty($pwdread) || !password_verify($_SERVER['PHP_AUTH_PW'], $pwdread['pass_hash'])){
        ...
        header('WWW-Authenticate: Basic realm="http://u67424.kubsu-dev.ru/back_web/task6/admin.php"');
