...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if ($errors['year']) {
    ...
    $messages['year_message'] = '<div class="error">Укажите год рождения</div>';
  }
  if ($errors['gender']) {
    setcookie('gender_error', '', 100000);
    $messages['gender_message'] = '<div class="error">Напишите ваш пол</div>';
  }

  if ($errors['super']) {
    setcookie('super_error', '', 100000);
    $messages[] = '<div class="error">Выберите хотя бы один яп</div>';
  }
  if ($errors['bio']) {
